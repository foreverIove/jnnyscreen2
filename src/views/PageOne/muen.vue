<template>
  ...
  <div class="center-top-left" ref="boxAll">
    <div v-show="menuShow" class="menus-container animate__animated"
      :class="menuShow ? 'animate__zoomInUp' : 'animate__zoomOut'" @mouseleave="hideSubmenu()">
      <div class="first-menus">
        <div class="menus-item" v-for="(item, index) in firstMenus" @click.stop="selectFirst(item, index)"
          @mouseover="overMenus(item, index)">
          {{ item.name }}
        </div>
        <div style="color: #fff; margin: 0 auto; position: absolute; bottom: 50px; left: 40px" @click="upmenu()">
          收起
        </div>
      </div>
      <!-- " -->
      <div class="second-menus" v-if="showSecond">
        <div class="menus-item" v-for="(item, index) in firstMenus[overFirstIndex]['children']" :key="index"
          @mouseover="overSecondMenus(item, index)" @click.stop="selectSecond(item, index)">
          {{ item.name }}
        </div>
      </div>
      <div class="second-menus" v-if="showThree">
        <div class="menus-item" v-for="(item, index) in firstMenus[overFirstIndex]['children'][overSecondIndex][
            'children'
          ]" :key="index" @click.stop="selectThree(item, index)">
          <el-tooltip v-if="item.name" :content="item.name" placement="right" style="width: 100%; overflow: hidden">
